...
char *gba_message = NULL;
char *gba_message_subject = NULL;
char *gba_message_recipient[] = {NULL,NULL,NULL};

volatile int sending_mail = 0;
...
*/
    ...
    int valid_recipients_count = 0;

    for (int i = 0; i < 3; i++) {
        if (gba_message_recipient[i] != NULL && strlen(gba_message_recipient[i]) > 0) {
            len = snprintf((char *) buf, BUF_SIZE, "RCPT TO:<%s>\r\n", gba_message_recipient[i]);
            ret = write_ssl_and_get_response(&ssl, (unsigned char *) buf, len);

            // If ret < 0, it's a socket/SSL crash. We must exit.
            if (ret < 0) {
            ESP_LOGE(TAG, "Network error during RCPT");
                goto exit;
            }

            // 250 is the SMTP "OK" code. 
            if (ret >= 200 && ret <= 299) {
                valid_recipients_count++;
            } else {
                ESP_LOGW(TAG, "Recipient %s rejected by server (Code %d)", gba_message_recipient[i], ret);
            }
        }
    }

    // Only proceed to DATA if at least one address was accepted
    if (valid_recipients_count == 0) {
        ESP_LOGE(TAG, "No valid recipients accepted. Aborting.");
        goto exit;
    }
    ...
    for (int i = 0; i < 3; i++) {
        if (gba_message_recipient[i] != NULL) {
            if (strlen(to_header) > 0) strcat(to_header, ", ");
            strcat(to_header, gba_message_recipient[i]);
        }
    }
    ...
    len = snprintf((char *) buf, BUF_SIZE,
                   ...
                   "\r\n", 
                   "Game Boy Advance & ESP32", to_header, gba_message_subject);
    ...
    /* Multipart boundary */
    len = snprintf((char *) buf, BUF_SIZE, "--XYZabcd1234\r\n");
    ret = write_ssl_data(&ssl, (unsigned char *) buf, len);

    /* Text */
    char sensor_readings[64];
    sprintf(sensor_readings,"\n\nTemperature: %d°C, Humidity: %d%%",last_temperature,last_humidity);
    len = snprintf((char *) buf, BUF_SIZE,
                   "--XYZabcd1234\r\n"
                   "Content-Type: text/plain; charset=UTF-8\r\n"
                   "\r\n" // Ends sub-header for this part
                   "%s%s\r\n", 
                   gba_message,sensor_readings);
    ret = write_ssl_data(&ssl, (unsigned char *) buf, len);
...
exit:
    ...
    for (int i = 0; i < 3; i++) {
        if (gba_message_recipient[i]) {
            free(gba_message_recipient[i]);
            gba_message_recipient[i] = NULL;
        }
    }
    uint64_t now = esp_timer_get_time();
    printf("\nTIME SPENT SENDING MAIL: %lld us \n",now-prev);
    sending_mail = 0;

            uint8_t val = 254; 
            printf("Email OPERATION COMPLETE\n");
